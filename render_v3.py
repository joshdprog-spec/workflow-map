...
def build_data(D, CFG, HERE, NOW, ROOT):
    ...
    outputs = [{"t": o["title"], "a": o["account"], "d": _iso(o["last"]), "folders": o["folders"], "dir": o["dir"], "files": o["files"]} for o in D.get("cowork_outputs", [])]
    ...
    return {
        ...
        "accounts": accounts, "projects": projects, "ledger": ledger, "loose": loose, "tasks": tasks, "outputs": outputs, "claude_projects": cps,
...
v.innerHTML='<h1>Where you left off</h1><p class="sub">Your most recently touched projects and the session to reopen in each. Copy a command, paste it in a terminal, and you are back inside that conversation.</p>'+banner+kpis+'<h2>Pick up here</h2><div class="list">'+rows+'</div><h2>Accounts on this PC</h2><div class="acct-cards">'+accts+'</div>';
...
v.innerHTML='<h1>Search: <span class="dim" style="font-weight:500">'+esc(v0)+'</span></h1><div class="two"><div><h2>Inside conversations <span id="deep-n" class="cnt">…</span></h2><div id="deep"><div class="dim">Searching…</div></div></div><div><h2>Projects <span class="cnt">'+pm.length+'</span></h2><div class="list">'+pm.slice(0,8).map(function(p){return '<div class="row" data-open="'+esc(p.name)+'" tabindex="0"><div><div class="head"><span class="name">'+esc(p.name)+'</span><span class="chip">'+esc(p.group)+'</span></div><div class="note">'+esc(p.note)+'</div></div><div></div></div>'}).join('')+'</div><h2>Session titles <span class="cnt">'+lm.length+'</span></h2>'+lm.slice(0,25).map(function(s){return '<div class="srow"><span class="when">'+esc((s.d||'').slice(0,10))+'</span><span class="what">'+kchip(s.k)+'<b>'+esc(s.t||'(untitled)')+'</b><span class="f">'+esc(s.f||'')+'</span></span><span>'+(s.r&&s.id.length>20?copyBtn('claude --resume '+s.id):'')+'</span></div>'}).join('')+'</div></div>';
...
var deep=document.getElementById('deep'),dn=document.getElementById('deep-n');
if(deepOK===false){deep.innerHTML='<div class="dim">Conversation search is not running. It starts with the next Claude session, or run <code>python "'+esc(M.here)+'\\search-server.py"</code>.</div>';dn.textContent='off';return}
fetch('http://127.0.0.1:'+M.port+'/search?q='+encodeURIComponent(v0),{mode:'cors'}).then(function(r){return r.json()}).then(function(res){deepOK=true;dn.textContent=res.length;
